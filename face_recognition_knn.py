...
in an unknown image by finding the k most similar faces(images with closet face-features under eucledian distance) in its training set,
...
import os
from math import sqrt  # sqrt = Square()
...
# Ball-Tree는 범위를 기준으로 차원을 내림차순 정렬한 후, 서브트리에 속한 범위의 반을 기준으로 KD-Tree를 적용
# 디렉토리, disk 모델 저장 경로, 분류에 가중치를 부여할 이웃 수, 기본 데이터 구조, 진행상황 보여줌 - x )
# knn : K-Nearest Neighbors ('K개의 최근접 이웃')
def train(train_dir, model_save_path="", n_neighbors=None, knn_algo='ball_tree', verbose=False):
    """
    Trains a k-nearest neighbors classifier for face recognition.
    # 이름과 함께 알려진 각 사람에 대한 하위 디렉토리를 포함하는 디렉토리.
    ...
    # 분류에 가중치를 부여할 이웃 수. 지정하지 않으면 자동으로 선택
    :param n_neighbors: (optional) number of neighbors to weigh in classification. Chosen automatically if not specified.
    # knn.default를 지원하는 기본 데이터 구조는 ball_tree
    :param knn_algo: (optional) underlying data structure to support knn.default is ball_tree
    :param verbose: verbosity of training
    # 주어진 데이터가 학습되는 knn 분류기 반환
    :return: returns knn classifier that was trained on the given data.
    """
    X = []
    y = []
    # dir( ) : 해당 객체가 어떤 변수와 메소드(method)를 가지고 있는지 나열/ 내장되어 있는 함수로 객체의 속성목록을 알려줌
    for class_dir in listdir(train_dir):
        # isdir() : 디렉토리 존재 여부 확인 - TRUE/False로 반환   # join() : 리스트를 문자열로 합쳐줌
        if not isdir(join(train_dir, class_dir)):
            continue
        # glob() : 인자로 받은 패턴과 이름이 일치하는 모든 파일과 디렉터리의 리스트를 반환 (패턴을 그냥 *라고 주면 모든 파일과 디렉터리)
        for img_path in sorted(glob(join(train_dir, class_dir, '*'))):
            image = face_recognition.load_image_file(img_path)
            faces_bboxes = face_locations(image)
            if len(faces_bboxes) != 1:
                if verbose:  # 학습 진행 상황 보여주는 것
    ...
    if n_neighbors is None:  # knn에서 k의 값을 정하지 않는다는 뜻 -> 임의로 선택
    ...
    if model_save_path != "":
        # model_save_path 파일을 새로 열어서 f라는 이름을 붙임. wb는 바이트(byte) 형식으로 쓰겠다(write)의 의미
        with open(model_save_path, 'wb') as f:
            # dump는 knn_clf를 파일 f에 기록. (파이썬이 실행되는 경로에 knn_clf파일이 만들어짐)
            pickle.dump(knn_clf, f)
    return knn_clf


# 거리 한계점 (distance_threshold= .4) - 숫자 클수록 학습된 사람으로 잘못 분류할 가능성 큼
def predict(X_img_path, knn_clf=None, model_save_path="", DIST_THRESH=.4):
    ...
    # knn 분류자 객체. 지정하지 않으면 model_save_path를 지정해야 함
    :param knn_clf: (optional) a knn classifier object. if not specified, model_save_path must be specified.
    # pickled knn 분류자의 경로. 지정되지 않은 경우 model_save_path는 knn_clf여야 함.
    :param model_save_path: (optional) path to a pickled knn classifier. if not specified, model_save_path must be knn_clf.
    # 크기가 클수록 알려지지 않은 사람을 알려진 사람으로 잘못 분류할 가능성이 커짐.
    ...
    if knn_clf is None and model_save_path == "":  # raise Exception : 오류 발생시 구문 출력
    ...
    print(X_img.shape)  # (1440, 1440, 3): 픽셀이미지 크기 1440 * 1440 , 3 = rgb(3color)
    ...
    if len(X_faces_loc) == 0:
        return []
    print(X_faces_loc[0])  # 얼굴 이미지 좌표(y1, x1, x0, y0)
...
def draw_preds(img_path, preds):
    ...
    :return:
    """
    source_img = Image.open(img_path).convert("RGBA")  # red green blue alpha
    draw = ImageDraw.Draw(source_img)

    # ex) item_list = [1, 2, 3, 4, 5]
...
if __name__ == "__main__":
    # train폴더에 학습시킬 이미지를 각각의 인물의 폴더로 구별해서 저장해야 나중에 그 폴더명으로 라벨링 처리됨
    knn_clf = train("./data/train", model_save_path='./models/fr_knn.pkl')  # train폴더에 학습시킬 이미지 폴더로 저장해야 나중에 라밸링 처리됨
    for img_path in listdir("./data/test"):
        # preds = predict(join("./data/test", img_path), knn_clf=knn_clf)
        preds = predict(join("./data/test", img_path), model_save_path='./models/fr_knn.pkl',
                        DIST_THRESH=0.4)  # 거리 한계점 (distance_threshold= 0.4)
