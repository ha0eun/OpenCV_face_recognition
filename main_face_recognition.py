...
#  dnn 모듈에서의 readNetFromCaffe 함수를 사용해,caffe framework로 해당 이미지를 학습, caffeframework의 네트워크 모델을 읽어들인다.
...
out = cv2.VideoWriter('1.avi', fcc, fps, (width, height))  # '저장할 파일명'
out_2 = cv2.VideoWriter('1_1.avi', fcc, fps, (width, height))
...
for idx in range(length):
    img_bgr = cap.read()[1]  # read로 한 프레임씩 불러왔음
    if img_bgr is None:
        break
    # if idx%3 != 0: continue
    # if idx < 200: continue
    ...
    img_bgr = preprocess(img_bgr)  # 이미지 전처리
    ...
    for name, bbox, conf in list_reconized_face:
        ...
        if name == 'unknown':
            ...
            # fx=.05, fy=.05 숫자가 클수록 픽셀 사이즈가 작아진다
